# SACH_HUB_SACH_VERTRAG.sql

drop table T_LVM_MART.SACH_HUB_SACH_VERTRAG;
drop table T_LVM_MART_EXT.EXT_SACH_HUB_SACH_VERTRAG;
drop table T_LVM_MART_WORK.TR_SACH_HUB_SACH_VERTRAG;
drop table T_LVM_MART_WORK.LD_SACH_HUB_SACH_VERTRAG;

CREATE MULTISET TABLE T_LVM_MART_EXT.EXT_SACH_HUB_SACH_VERTRAG ,FALLBACK ,
NO BEFORE JOURNAL,
NO AFTER JOURNAL,
CHECKSUM = DEFAULT,
DEFAULT MERGEBLOCKRATIO,
MAP = TD_MAP1
(
PRIMARY_KEY DECIMAL(20, 0) NOT NULL,
VSNR VARCHAR(50) CHARACTER SET LATIN CASESPECIFIC NOT NULL,
VG_SICHT_ZP TIMESTAMP(6) NOT NULL,
GUELTIG_VON_ZP TIMESTAMP(6) NOT NULL,
AS_LFD_NR SMALLINT,
VS_BEDINGUNG_GEN SMALLINT,
VERTRAGSART_SCHL SMALLINT,
VERTRAG_STATUS_SCHL SMALLINT,
VG_GUELT_BEG_DAT DATE FORMAT 'YY/MM/DD',
ARBEITSSCHRITT_SCHL SMALLINT,
VS_BEDINGUNG_FAM SMALLINT,
HAUPTFAELLIGKEIT_MONAT SMALLINT,
HAUPTFAELLIGKEIT_TAG SMALLINT,
BEIT_GEM_ZW_BRUTTO DECIMAL(14,2),
BEIT_GEM_ZW_NETTO DECIMAL(14,2),
EINWILLIGUNSERKL_ART SMALLINT,
IDENTIFIZIERUNG_KTXT VARCHAR(50) CHARACTER SET LATIN CASESPECIFIC,
J_BEIT_BRUTTO DECIMAL(14,2),
J_BEIT_NETTO DECIMAL(14,2),
J_BEIT_NETTO_O_ZW DECIMAL(14,2),
VN_PA_REF_NR VARCHAR(50) CHARACTER SET LATIN CASESPECIFIC,
PROD_LINIE SMALLINT,
VTR_SB_BETR SMALLINT,
VTR_SB_PROZ SMALLINT,
STORNIERER SMALLINT,
TARIFMERKMAL SMALLINT,
VS_BEGINN_DAT DATE FORMAT 'YY/MM/DD',
VS_ENDE_DAT DATE FORMAT 'YY/MM/DD',
VN_ART_SCHL SMALLINT,
VS_STEUER_BETR DECIMAL(14,2),
VS_STEUER_GEM_ZW DECIMAL(14,2),
VS_ZWEIG SMALLINT,
ZW SMALLINT,
GUELTIG_BIS_ZP TIMESTAMP(6) NOT NULL,
VERARBEITUNG_ZP TIMESTAMP(6) NOT NULL
)
NO PRIMARY INDEX;
CREATE MULTISET TABLE T_LVM_MART_WORK.TR_SACH_HUB_SACH_VERTRAG ,FALLBACK ,
NO BEFORE JOURNAL,
NO AFTER JOURNAL,
CHECKSUM = DEFAULT,
DEFAULT MERGEBLOCKRATIO,
MAP = TD_MAP1
(
PRIMARY_KEY DECIMAL (20, 0) NOT NULL,
VSNR VARCHAR(50) CHARACTER SET LATIN CASESPECIFIC NOT NULL,
AS_LFD_NR SMALLINT NOT NULL,
VG_SICHT_ZP TIMESTAMP(6) NOT NULL,
LOADDATE TIMESTAMP(6) NOT NULL,
SOURCE_SYS VARCHAR (50) DEFAULT 'PostgreSQL'
)
PRIMARY INDEX( VSNR, VG_SICHT_ZP, AS_LFD_NR );
CREATE MULTISET TABLE T_LVM_MART_WORK.LD_SACH_HUB_SACH_VERTRAG ,FALLBACK ,
NO BEFORE JOURNAL,
NO AFTER JOURNAL,
CHECKSUM = DEFAULT,
DEFAULT MERGEBLOCKRATIO,
MAP = TD_MAP1
(
PRIMARY_KEY DECIMAL (20, 0) NOT NULL,
VSNR VARCHAR(50) CHARACTER SET LATIN CASESPECIFIC NOT NULL,
AS_LFD_NR SMALLINT NOT NULL,
VG_SICHT_ZP TIMESTAMP(6) NOT NULL,
LOADDATE TIMESTAMP(6) NOT NULL,
SOURCE_SYS VARCHAR (50),
VERARBEITUNG_ZP TIMESTAMP(6) NOT NULL
)
PRIMARY INDEX( VSNR, VG_SICHT_ZP, AS_LFD_NR );
CREATE MULTISET TABLE T_LVM_MART.SACH_HUB_SACH_VERTRAG ,FALLBACK ,
NO BEFORE JOURNAL,
NO AFTER JOURNAL,
CHECKSUM = DEFAULT,
DEFAULT MERGEBLOCKRATIO,
MAP = TD_MAP1
(
PRIMARY_KEY DECIMAL (20, 0) NOT NULL,
VSNR VARCHAR(50) CHARACTER SET LATIN CASESPECIFIC NOT NULL,
AS_LFD_NR SMALLINT NOT NULL,
VG_SICHT_ZP TIMESTAMP(6) NOT NULL,
LOADDATE TIMESTAMP(6) NOT NULL,
SOURCE_SYS VARCHAR (50),
VERARBEITUNG_ZP TIMESTAMP(6) NOT NULL
)
PRIMARY INDEX( VSNR, VG_SICHT_ZP, AS_LFD_NR );